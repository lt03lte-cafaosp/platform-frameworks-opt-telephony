...
public class SmsMessage extends SmsMessageBase {
     ...
     * @hide
     */
    public static DeliveryPdu getDeliveryPdu(String scAddress, String destinationAddress,
            String message, boolean statusReportRequested, byte[] header, byte[] date) {
        // Perform null parameter checks.
        if (destinationAddress == null) {
            return null;
        }
        if (message == null) {
            return getDeliveryPduEmptyMessage(scAddress, destinationAddress,
                    statusReportRequested, header, date);
        }

        DeliveryPdu ret = new DeliveryPdu();
        // MTI = SMS-DELIVER
        byte mtiByte = (byte)(0x00);
        ByteArrayOutputStream bo = getDeliveryPduHead(scAddress, destinationAddress, mtiByte,
                statusReportRequested, ret);
        try {
    ...
    */
    private static DeliveryPdu getDeliveryPduEmptyMessage(String scAddress,
                String destinationAddress, boolean statusReportRequested,
    ...
    *  address, if applicable, and the encoded message
    */
    private static ByteArrayOutputStream getDeliveryPduHead(String scAddress,
