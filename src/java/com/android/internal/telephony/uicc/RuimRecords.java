...
public final class RuimRecords extends IccRecords {
     ...
     * CSIM flags
     */
    public static boolean omhEnabled = false;
    private static boolean mMMSICPEnabled = false;
    ...
    /**
     * OMH related EF elementary file events
     */
    private static final int EVENT_GET_SSFC_DONE = 32;
    private static final int EVENT_GET_ECC_LIST_DONE = EVENT_GET_SSFC_DONE + 1;
    private static final int EVENT_GET_MMS_ICP_DONE = EVENT_GET_ECC_LIST_DONE + 1;
    private static final int EVENT_GET_LBS_XTRA_CONFIG_DONE = EVENT_GET_MMS_ICP_DONE + 1;
    private static final int EVENT_GET_LBS_XTRA_SERVURL_DONE = EVENT_GET_LBS_XTRA_CONFIG_DONE + 1;
    private static final int EVENT_GET_LBS_V2_CONFIG_DONE = EVENT_GET_LBS_XTRA_SERVURL_DONE + 1;
    private static final int EVENT_GET_LBS_V2_PDE_ADDR_DONE = EVENT_GET_LBS_V2_CONFIG_DONE + 1;
    private static final int EVENT_GET_LBS_V2_MPC_ADDR_DONE = EVENT_GET_LBS_V2_PDE_ADDR_DONE + 1;
    private static final int EVENT_GET_SMSCAP_DONE = EVENT_GET_LBS_V2_MPC_ADDR_DONE + 1;
    private static final int EVENT_GET_CSIM_MMS_CONFIG = EVENT_GET_SMSCAP_DONE + 1;
    ...
    private class EfRuimIdLoaded implements IccRecordLoaded {
        ...
        public void onRecordLoaded(AsyncResult ar) {
            ...
            byte[] data = (byte[])ar.result;
            if (DBG) log("RuimId Data="+IccUtils.bytesToHexString(data));
    ...
    public void handleMessage(Message msg) {
        ...
        try { switch (msg.what) {
            ...
            case EVENT_SET_MODEL_DONE:
                ar = (AsyncResult) msg.obj;
                if (ar.exception != null) {
                    Log.i(LOG_TAG, "Set EF Model failed", ar.exception);
                }
                Log.d(LOG_TAG, "EVENT_SET_MODEL_DONE");
                break;
            case EVENT_GET_RUIM_CST_DONE: {
                ...
                if (ar != null && ar.exception == null) {
                    data = (byte[]) ar.result;
                    Log.d(LOG_TAG, "EF CST data: " + IccUtils.bytesToHexString(data));

                    // for CSIM card
                    if (null != mParentApp && AppType.APPTYPE_CSIM == mParentApp.getType()) {
                        Log.d(LOG_TAG, "CSIM card type");
                        Log.d(LOG_TAG, "CSIM EF CST data data[4]: " + data[4]);
                        ...
                        if (omhEnabled) {
                            ...
                            mMMSICPEnabled = (0x04 == (0x04 & data[2])) ? true : false;
                        }

                    } else {
                        Log.d(LOG_TAG, "OMH EF CST data data[3]: " + data[3]);
                        omhEnabled = (0x30 == (0x30 & data[3])) ? true : false;
                        /**
                         * judge if read MMSICP flag is enabled on CST
                         */
                        if (omhEnabled) {
                            mMMSICPEnabled = (0x0C == (0x0C & data[10])) ? true : false;
                        }
                    }

                    Log.d(LOG_TAG, "mms icp enabled =" + mMMSICPEnabled + " omhEnabled " + omhEnabled);
    ...
    private void setModel() {
        ...
        try{
            ...
            byte[] model = str.getBytes("UTF-8");
            /**
             * restrict model length to prevent ArrayIndexOutOfBoundsException
             */
            int modelLen = (model.length > MODEL_INFORMATION_SIZE) ? MODEL_INFORMATION_SIZE
                    : model.length;
            System.arraycopy(model, 0, data, 2, modelLen);

            str = Build.MANUFACTURER;
            byte[] manufacturer = str.getBytes("UTF-8");
            /**
             * restrict manufacture length to prevent ArrayIndexOutOfBoundsException
             */
            int manufactureLen = (manufacturer.length > MANUFACTURER_NAME_SIZE) ? MANUFACTURER_NAME_SIZE
                    : manufacturer.length;
            System.arraycopy(manufacturer, 0, data, 2 + MODEL_INFORMATION_SIZE, manufactureLen);
             ...
             * it will leads to arrayoutofindex exception
             */
            int versionLength = (softwareVersion.length > SOFTWARE_VERSION_INFORMATION_SIZE) ? SOFTWARE_VERSION_INFORMATION_SIZE
                    : softwareVersion.length;
            System.arraycopy(softwareVersion, 0, data, 2 + MODEL_INFORMATION_SIZE
                    + MANUFACTURER_NAME_SIZE, versionLength);
            ...
            Log.e(LOG_TAG, "BearerData encode failed: " + e);
        }

        if (null != mParentApp && AppType.APPTYPE_CSIM == mParentApp.getType()) {
            Log.d(LOG_TAG, "CSIM card type,start set csim model");
