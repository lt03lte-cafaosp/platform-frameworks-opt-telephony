...
public class MSimSmsManager {
     ...
     * {@hide}
     */
    public void sendMultipartTextMessageWithPriority(String destinationAddress,
        ...
        if (parts.size() > 1) {
            try {
                ISmsMSim iccISms = ISmsMSim.Stub.asInterface(
                        ServiceManager.getService("isms_msim"));
                if (iccISms != null) {
                    iccISms.sendMultipartTextWithPriority(ActivityThread.currentPackageName(),
                            destinationAddress, scAddress, parts, sentIntents, deliveryIntents,
    ...
    public int copyMessageToIccGetEfIndex(byte[] smsc, byte[] pdu, int status,
        ...
        try {
            ISmsMSim iccISms = ISmsMSim.Stub.asInterface(ServiceManager.getService("isms_msim"));
            if (iccISms != null) {
                index = iccISms.copyMessageToIccGetEfIndex(status, pdu,
                                  smsc, subscription);
           }
        } catch (RemoteException ex) {
            // ignore it
   ...
   /**
    ...
    public static int getSmsCapacityOnIcc(int subscription) {
        ...
        try {
            ...
            if (simISms != null) {
                ret = simISms.getSmsCapacityOnIcc(subscription);
            }
        } catch (RemoteException ex) {
            //ignore it
        }
